    ...
    public class Blit : ScriptableRendererFeature
    {
        public class BlitPass : ScriptableRenderPass
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                source = settings.srcType switch
                {
                    // note : Seems this has to be done in here rather than in AddRenderPasses to work correctly in 2021.2+
                    Target.CameraColor => renderer.cameraColorTarget,
                    Target.TextureID => new RenderTargetIdentifier(settings.srcTextureId),
                    Target.RenderTextureObject => new RenderTargetIdentifier(settings.srcTextureObject),
                    _ => source
                };

                destination = settings.dstType switch
                {
                    Target.CameraColor => renderer.cameraColorTarget,
                    Target.TextureID => new RenderTargetIdentifier(settings.dstTextureId),
                    Target.RenderTextureObject => new RenderTargetIdentifier(settings.dstTextureObject),
                    _ => destination
                };
